...
**Seventh revision note (G-A/R1 proof-ownership collision repair,
2026-09-12). OWNER-CONFIRMED 2026-09-12.** The accepted G-A RED probe,
SHA-256
`d224a52544dde336604abbdcb246fcc9eed9a46762777bff37b6ab8dab345952`,
proved that §52's original G-A ownership was impossible: G-A precedes and
forbids production provider implementation, while `P2-EVT-009` and
`P2-VM-004` require actual production consumers and composition.

Contract §53 preserves both requirements verbatim and transfers them atomically
to the newly defined `phase2-io-r1-evidence-chain` gate. It mints
`P2-EVT-011` and `P2-VM-006` for G-A's earlier governed route decision and
production-equivalent-sandbox candidate proof. G-B and G-C ownership is
unchanged. R1 depends on all three R0 gates. This repair authorizes no
production scope: the R1 manifest must receive a separately reviewed amendment
naming exact production files after G-A's accepted ADR/source matrix exists.

The repaired boundary is:

```text
G-A / R0: candidate access feasibility + completeness contract
    →
R1: implementation + actual packaged production composition proof
```

Missing production wiring alone is neither rejection nor unavailability. G-A
may not invent helper authority or relax the accepted sandbox. Universal
production-reachability remains Protocol-owned and ID-less. The Master-Spec
outcome and R0-to-R1 dependency direction do not change.
...
| P2-VM-004 *(new, R0-GOV; OWNER-CONFIRMED 2026-09-11; ownership transferred unchanged to R1 by §53/§19 seventh revision)* | Real disposable/reference-environment evidence MUST prove the selected G-A I/O evidence routes from the actual packaged production daemon/helper topology with the accepted daemon sandbox active and unchanged. Host-shell-only reachability is insufficient. The proof MUST exercise the real upstream producer, production consumer, production cadence, provider topology, sandbox, and lifecycle applicable to each source, including its partial, unavailable, or unknown visibility behavior. |
